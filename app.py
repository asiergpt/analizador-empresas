...
def clean_number_format(val):
    ...
    # 1. Si es un RANGO (ej: 130-150), lo devolvemos tal cual.
    # La regex verifica que haya un guion en medio.
    if re.search(r'\d+[-–]\d+', val_str):
        return val_str
    
    # 2. Si tiene símbolos al inicio (>, <, +), lo devolvemos tal cual.
    if re.match(r'^[><+]', val_str):
        return val_str
    
    # 3. Si es un número simple, lo formateamos.
    # Usamos try/except para asegurar que es un entero limpio.
    try:
        # Quitamos posibles puntos antes de convertir a int si es necesario, 
        # aunque standarize ya debería haberlos quitado.
        val_limpio = val_str.replace('.', '')
        numero = int(val_limpio)
        return "{:,}".format(numero)
    except ValueError:
        pass # No era un número entero simple
...
def uses_ai(value):
    ...
    # Valores que significan SÍ
    yes_values = {'sí', 'si', 'yes', 'verdadero', 'activo'}
    
    # Valores que significan NO
    no_values = {'no', 'n', 'false', '0', 'falso', 'inactivo', 'nan', '-', ''}
    
    # Si está en los valores de SÍ
    if val_str in yes_values:
        return True
    # Si está en los valores de NO
    elif val_str in no_values:
        return False
    # Si contiene "sí" o "yes" (búsqueda parcial)
    else:
        return 'sí' in val_str or 'yes' in val_str or 'si ' in val_str or ' si' in val_str
...
def standarize_numero_empleados(x):
    ...
    - Mantiene rangos (130-150) sin comillas.
    - Corrige el error de "16.333" -> "16" (ahora devuelve "16333").
    - Convierte texto como "Menos de" a símbolos (<).
    """
    if pd.isna(x):
        return "0"
    ...
    # 1. Limpieza: Quitar paréntesis con texto extra (ej: "(Wikipedia)")
    val_str = re.sub(r'\s*\(.*?\)', '', val_str).strip()
    
    # Si tras limpiar no hay dato válido
    if any(k in val_str.lower() for k in ['sin datos', 'no disponible', 'desconocido', 'n/a']):
        return "0"

    # 2. Normalizar "Menos de" / "Más de" a símbolos
    ...
    # CASO A: Rango (ej: "130-150")
    # Busca números (con o sin puntos) separados por guion
    rango_match = re.search(r'([\d\.]+)\s*[-–]\s*([\d\.]+)', val_str)
    if rango_match:
        start = rango_match.group(1).replace('.', '') # Quitamos puntos de miles
        end = rango_match.group(2).replace('.', '')
        return f"{start}-{end}" # SIN comillas

    # CASO B: Desigualdad (ej: ">1.300", "<10")
    ...
    # CASO C: Número suelto (ej: "16.333", "~500")
    # AQUÍ ESTABA EL ERROR: Usamos [\d\.]+ para capturar dígitos Y puntos
    num_match = re.search(r'([\d\.]+)', val_str)
    if num_match:
        clean_num = num_match.group(1).replace('.', '') # 16.333 -> 16333
        return clean_num
...
if st.session_state.page == 'home':
    ...
    <div class="hero-section">
        <h1>🎯 BINGO</h1>
        <p style="font-size: 1.3em; margin: 15px 0 0 0; opacity: 0.95; font-weight: 300;">Filtro empresas, conecto con profesionales y ¡canto bingo!</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if df_personas.empty:
        ...
        if not df_page.empty:
            for index, row in df_page.iterrows():
                ...
                # 1. Tu función ya hace la prioridad (1º Einforma, 2º DBA)
                empresa = get_empresa_name(row) 
                
                # Opcional: Si quieres que salga en mayúscula/bonito, puedes usar capitalize_first_letter(empresa)
                # Pero a veces las empresas tienen siglas (SaaS), así que mejor dejarlo tal cual o limpiarlo un poco.
                
                provincia = "-"
                jerarquia = safe_get_val(row, 'jerarquía')
                funcion = capitalize_first_letter(safe_get_val(row, 'función'))
                
                # Obtener provincia desde df_main para mostrarla dentro (esto ya lo tenías)
                if empresa != "-":
                ...
                # 2. CAMBIO VISUAL AQUÍ: Añadimos la empresa al título del desplegable
                # Usamos iconos y separadores (|) para que quede ordenado.
                titulo_tarjeta = f"👤 **{nombre}** |  🏢 **{empresa}** |  🛠 {funcion}"
