...
# --- 1. GESTIÓN DE ESTILOS (CSS PREMIUM) ---
def inject_css():
    st.markdown("""
    <style>
...
# ----------------- DETALLE (ESTRUCTURA SOLICITADA) -----------------
    ...
    if not row_data.empty:
        ...
        <div class="content-box">
            <span class="box-header">Sector: {get_val(r, 'SECTOR_NOMBRE')}</span>
            <span class="box-text"><b>Actividad Principal:</b> {get_val(r, 'actividad_resumen')}</span>
        ...
        if col_matriz and not df_alumni.empty:
            ...
            if not match.empty:
                ...
                with t1:
                    # Lógica de ordenamiento Top -> Middle -> Entry
                    if 'jerarquía' in match.columns:
                        ...
                        # Añadir el order de nuevo al dataframe agrupado para ordenar
                        counts['sort_key'] = counts['jerarquía'].map(order_map).fillna(5)
