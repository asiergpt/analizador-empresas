...
def inject_css():
    ...
    .badge-top { 
        background-color: #1F4E79; /* Corporate Blue */
        color: white; 
        padding: 5px 10px; 
        border-radius: 4px; 
        font-size: 0.85rem; 
...
# ----------------- DETALLE -----------------
    ...
    if not row_data.empty:
        ...
        if not match.empty:
            ...
            # Preparar datos ordenados para AMBOS tabs
            if 'jerarquía' in match.columns:
                # Mapa de ordenamiento estricto
                order_map = {'Top Management': 1, 'Middle Management': 2, 'Entry Level': 3, 'Entry Level/Others': 3, 'Otros': 4}
                
                # Crear copia y columna de ordenamiento
                df_sorted = match.copy()
                df_sorted['sort_val'] = df_sorted['jerarquía'].map(order_map).fillna(5)
                
                # Ordenar el DataFrame completo por Jerarquía -> Nombre
                df_sorted = df_sorted.sort_values(by=['sort_val', 'Nombre'])
            else:
                df_sorted = match.copy()

            with t1:
                if 'jerarquía' in match.columns:
                    # Agrupar preservando el orden definido
                    counts = df_sorted.groupby(['jerarquía', 'sort_val']).size().reset_index(name='Total')
                    counts = counts.sort_values('sort_val')
                    
                    html_sum = '<table class="custom-table" style="width:100%"><thead><tr><th>Nivel</th><th>Total</th></tr></thead><tbody>'
                    for _, row_c in counts.iterrows():
                        cat = row_c['jerarquía']
                        badge = "badge-top" if "Top" in str(cat) else "badge-mid" if "Middle" in str(cat) else "badge-entry"
            ...
            with t2:
                ...
                render_table(df_sorted[cols_exist])
        else:
