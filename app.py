...
# --- 7. PÁGINA: EXPLORADOR ---
    ...
    # NUEVO: Filtro Deusto Alumni (Basado en conteo de contactos)
    if st.session_state.f_deusto == "Sí":
        # Filtrar empresas que TIENEN al menos 1 contacto Deusto
        empresas_con_deusto = set()
        if not df_alumni.empty:
            for empresa_nombre in df_show['Nombre'].unique():
                match = df_alumni[df_alumni['nombre_matriz_einforma'].astype(str).str.strip() == str(empresa_nombre).strip()]
                if match.empty:
                    match = df_alumni[df_alumni['nombre_matriz_einforma'].astype(str).str.contains(re.escape(str(empresa_nombre).strip()), case=False, na=False)]
                
                # Si hay contactos Y al menos uno es de Deusto
                if len(match) > 0:
                    deusto_count = match[match['universidad'].astype(str).str.contains('deusto', case=False, na=False)]
                    if len(deusto_count) > 0:
                        empresas_con_deusto.add(empresa_nombre)
        
        df_show = df_show[df_show['Nombre'].isin(empresas_con_deusto)]

    elif st.session_state.f_deusto == "No":
        # Filtrar empresas que NO TIENEN contactos Deusto
        empresas_sin_deusto = set()
        if not df_alumni.empty:
            for empresa_nombre in df_show['Nombre'].unique():
                match = df_alumni[df_alumni['nombre_matriz_einforma'].astype(str).str.strip() == str(empresa_nombre).strip()]
                if match.empty:
                ...
                # Si NO hay contactos Deusto (0 contactos o ninguno es de Deusto)
                deusto_count = match[match['universidad'].astype(str).str.contains('deusto', case=False, na=False)] if len(match) > 0 else pd.DataFrame()
                if len(deusto_count) == 0:
                    empresas_sin_deusto.add(empresa_nombre)
        
        df_show = df_show[df_show['Nombre'].isin(empresas_sin_deusto)]
