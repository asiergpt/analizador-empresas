...
# ----------------- DETALLE (DISEÑO ORIGINAL RESTAURADO) -----------------
elif st.session_state.page == 'detail':
    ...
    if not row_data.empty:
        ...
        if col_matriz and not df_alumni.empty:
            ...
            if not match.empty:
                ...
                with t1:
                    # Lógica robusta de ordenamiento
                    if 'jerarquía' in match.columns:
                        ...
                        temp_match['sort_val'] = temp_match['jerarquía'].map(order_map).fillna(5)
                        
                        # Agrupamos y ordenamos
                        cts = temp_match['jerarquía'].value_counts().reset_index()
                        cts.columns = ['Nivel', 'Total']
                        cts['s'] = cts['Nivel'].map(order_map).fillna(5)
                        cts = cts.sort_values('s').drop('s', axis=1)
                        
                        render_table(cts)
                    else:
                        st.write("Columna jerarquía no encontrada.")
