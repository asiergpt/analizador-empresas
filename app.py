...
def clean_number_format(val):
    ...
    # Si detectas una barra, probablemente son dos datos (ej: "5 / 250"). Devuélvelo tal cual.
    if '/' in val_str:
        return val_str
        # ---------------------------
...
def standarize_numero_empleados(x):
    """
    Estandariza empleados detectando puntos de miles, rangos y símbolos.
    Versión limpia para Web: Sin comillas y con mejor detección de texto.
    """
    if pd.isna(x):
        return "0"
    ...
    # 1. Limpieza básica: Eliminar paréntesis con fuentes (ej: "(Wikipedia)")
    val_str = re.sub(r'\s*\(.*?\)', '', val_str).strip()
    
    # Si tras limpiar es "Sin datos" o similar, devolver 0
    if any(k in val_str.lower() for k in ['sin datos', 'no disponible', 'desconocido', 'n/a']):
        return "0"

    # 2. Convertir texto "Menos de" / "Más de" a símbolos
    # IMPORTANTE: Reemplazamos el texto, no solo lo añadimos al principio
    ...
    # --- LÓGICA DE EXTRACCIÓN (Sin comillas) ---

    # CASO A: RANGOS (ej: "130-150", "1.000 - 2.000")
    # Captura números con o sin puntos separados por guion
    rango_match = re.search(r'([\d\.]+)\s*[-–]\s*([\d\.]+)', val_str)
    if rango_match:
        start = rango_match.group(1).replace('.', '') # Quitar punto de miles
        end = rango_match.group(2).replace('.', '')
        return f"{start}-{end}" # SIN comilla al inicio

    # CASO B: DESIGUALDADES (ej: ">1.300", "<10", "+500")
    # Busca símbolo seguido de número
    ineq_match = re.search(r'([><+])\s*([\d\.]+)', val_str)
    if ineq_match:
        symbol = ineq_match.group(1)
        num = ineq_match.group(2).replace('.', '')
        return f"{symbol}{num}" # SIN comilla al inicio

    # CASO C: NÚMERO SUELTO (ej: "16.333", "~500")
    # Busca cualquier secuencia de dígitos y puntos
