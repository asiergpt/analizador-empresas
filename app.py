...
def clean_number_format(val):
    ...
    # Si es un rango "130-150", "20-49", etc., devolvemos tal cual
    if re.match(r'^\d+-\d+$', val_str):
        return val_str
    
    # Si empieza con símbolo (>1300, <10, +7000), devolvemos tal cual
    if re.match(r'^[><+]\d+', val_str):
        return val_str
    
    # Si es solo números o tiene paréntesis, extraemos y formateamos
    numero_match = re.search(r'^(\d+)', val_str)
    if numero_match:
...
def standarize_numero_empleados(x):
    """
    Estandariza el número de empleados según criterios:
    - Rango: "20-49"
    - Símbolos: ">1300", "<10", "+7000"
    """
    if pd.isna(x):
        return "0"
    ...
    if any(keyword in val_lower for keyword in [
        'sin datos', 'no identificado', 'no disponible', 
        'no concluyente', 'no se encontr', 'desconocido',
        'no aplica', 'holding', 'dato no disponible', 
        'sin datos públicos', 'no encontrado'
    ]):
        return "0"
    
    # ============ CASO 2: Categorías España ============
    if 'microempresa' in val_lower:
        return "0-9"
    if 'pequeña' in val_lower and 'empresa' in val_lower:
        return "10-49"
    if 'mediana' in val_lower and 'empresa' in val_lower:
        return "50-249"
    if 'gran' in val_lower and 'empresa' in val_lower:
        return "≥250"
    ...
    if rango_match:
        ...
        # ⭐ CLAVE: Retornar como string con comilla para evitar conversión
        return f"'{inicio}-{fin}"  # La comilla fuerza string en pandas
    
    # ============ CASO 4: Símbolos al inicio (>, <, +) ============
    if re.match(r'^[><+]\d+', val_str):
        simbolo_match = re.match(r'^([><+])(\d+)', val_str)
        if simbolo_match:
            simbolo = simbolo_match.group(1)
            numero = simbolo_match.group(2)
            return f"'{simbolo}{numero}"  # ⭐ También aquí
    
    # ============ CASO 5: Número con "~" (aproximado) ============
    if '~' in val_str or 'aproximad' in val_lower or 'estim' in val_lower:
        numero_match = re.search(r'(\d+)', val_str)
        if numero_match:
            return numero_match.group(1)
    
    # ============ CASO 6: Número simple con anotaciones ============
    numero_simple = re.match(r'^(\d+)', val_str)
    if numero_simple:
        return numero_simple.group(1)
...
# --- 8. PÁGINA: DETALLE ---
    ...
    if not row_data.empty:
        ...
        st.write("")
        empleados_raw = safe_get_val(r, 'numero_empleados')
        st.write(f"**DEBUG - Empleados RAW:** `{empleados_raw}`")
        st.write(f"**DEBUG - Tipo:** {type(empleados_raw)}")
        st.write(f"**DEBUG - Repr:** {repr(empleados_raw)}")

        render_table(pd.DataFrame({
            "Ventas Est.": [clean_number_format(safe_get_val(r, 'ventas_estimado'))],
            "Empleados": [empleados_raw],
        ...
        render_table(pd.DataFrame({
            "Ventas Est.": [clean_number_format(safe_get_val(r, 'ventas_estimado'))],
            "Empleados": [safe_get_val(r, 'numero_empleados').lstrip("'")], 
            "Patentes": [str(r.get('patentes', 0))],
            "Año Const.": [safe_get_val(r, 'ano_constitucion')],
            "Web": [safe_get_val(r, 'web_oficial')]
        }))
