...
import os
import math
import random # Necesario para generar alumni mock dinámico
...
# --- 1. GESTIÓN DE ESTILOS (CSS PREMIUM REFINADO) ---
def inject_css():
    st.markdown("""
    <style>
    ...
    /* Content Box Ajustado para altura igual */
    .content-box { 
        ...
        padding: 20px; 
        height: 100%; 
        min-height: 300px; /* AUMENTADO PARA IGUALAR ALTURAS VISUALMENTE */
        display: flex;
...
def generate_mock_data():
    # Datos básicos de empresa por si no hay CSV
    data_emp = {
...
def generate_mock_alumni(df_empresas):
    """Genera alumni mock basados en las empresas cargadas para que el networking siempre funcione."""
    mock_names = ['Iker D.', 'Amaia L.', 'Unai E.', 'Maria S.', 'Jon A.', 'Mikel O.', 'Ane I.', 'Xabier P.']
    mock_roles = ['Developer', 'CTO', 'Product Manager', 'Director', 'Ingeniero', 'HR Manager', 'CEO']
    mock_levels = ['Middle Management', 'Top Management', 'Entry Level', 'Top Management', 'Middle Management']
    
    alumni_list = []
    
    # Si hay empresas cargadas, generamos alumni para ellas
    if not df_empresas.empty:
        nombres_empresas = df_empresas['Nombre'].unique().tolist()
        # Generar unos 50 alumni aleatorios repartidos
        for _ in range(50):
            empresa = random.choice(nombres_empresas)
            alumni_list.append({
                'Nombre': random.choice(mock_names),
                'Cargo': random.choice(mock_roles),
                'nombre_matriz_einforma': empresa,
                'jerarquía': random.choice(mock_levels),
                'función': 'Ingeniería',
                'url_linkedin': '#'
            })
    
    return pd.DataFrame(alumni_list)
...
def load_data():
    ...
    # Si no hay empresas cargadas, usar mock
    if df_main is None or df_main.empty:
        df_main = generate_mock_data()

    # Si no se cargó alumni real (o falló), generar mock basado en las empresas disponibles
    if not loaded_alumni or df_alumni.empty:
        df_alumni = generate_mock_alumni(df_main)
...
# ----------------- DETALLE -----------------
    ...
    if not row_data.empty:
        ...
        st.write("")
        # TABLA ORIGINAL CON AÑADIDO DE PATENTES
        ...
        # Layout equilibrado
        c_sec, c_act = st.columns([1, 1])
        with c_sec:
             st.markdown(f"""<div class="content-box"><div style="margin-bottom:10px;"><b>Sector:</b> {get_val(r, 'SECTOR_NOMBRE')}</div></div>""", unsafe_allow_html=True)
        with c_act:
             st.markdown(f"""<div class="content-box"><div><b>Actividad Principal:</b> {get_val(r, 'actividad_resumen')}</div></div>""", unsafe_allow_html=True)

        # 3. PROPIEDAD Y SOLVENCIA
        st.markdown('<div class="section-title">Propiedad y Solvencia</div>', unsafe_allow_html=True)
        c_prop, c_solv = st.columns(2)
        # Altura mínima forzada en CSS (.content-box: min-height: 300px)
        c_prop.markdown(f"""<div class="content-box"><span class="box-header">Estructura de la Propiedad</span><b>Accionistas:</b> {get_val(r, 'propiedad_accionistas')}<br><br><b>Private Equity:</b> {get_val(r, 'private_equity_firmas')}</div>""", unsafe_allow_html=True)
        c_solv.markdown(f"""<div class="content-box"><span class="box-header">Finanzas y Solvencia</span><b>Financiación Pública:</b> {get_val(r, 'financiacion_publica_detalle')}<br><br><b>Solvencia:</b> {get_val(r, 'solvencia_txt')}</div>""", unsafe_allow_html=True)
        ...
        # Filtro de empresa para alumni
        match = pd.DataFrame()
        if not df_alumni.empty:
            # Primero intentar coincidencia exacta con nombre de empresa
            match = df_alumni[df_alumni['nombre_matriz_einforma'].astype(str).str.strip() == str(selected_empresa).strip()]
            
            # Si no hay exacta, intentar contains (para el mock data generado aleatoriamente)
            if match.empty:
        ...
        if not match.empty:
            ...
            with t1:
                if 'jerarquía' in match.columns:
                    ...
                    counts = counts.sort_values('order').drop('order', axis=1)
                    render_table(counts)
