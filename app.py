...
# --- 7. PÁGINA: EXPLORADOR ---
    ...
    # NUEVO: Filtro Deusto Alumni (Usa la MISMA lógica de conteo que en detalle)
    if st.session_state.f_deusto == "Sí":
        # Filtrar empresas que TIENEN al menos 1 contacto
        ...
        if not df_alumni.empty:
            for empresa_nombre in df_show['Nombre'].unique():
                ...
                # Si hay contactos (sin filtrar por columna universidad)
                if len(match) > 0:
                    empresas_con_contactos.add(empresa_nombre)
        
        df_show = df_show[df_show['Nombre'].isin(empresas_con_contactos)]

    elif st.session_state.f_deusto == "No":
        # Filtrar empresas que NO TIENEN contactos
        empresas_sin_contactos = set()
        if not df_alumni.empty:
            for empresa_nombre in df_show['Nombre'].unique():
                match = df_alumni[df_alumni['nombre_matriz_einforma'].astype(str).str.strip() == str(empresa_nombre).strip()]
                if match.empty:
                ...
                # Si NO hay contactos
                if len(match) == 0:
                    empresas_sin_contactos.add(empresa_nombre)
        
        df_show = df_show[df_show['Nombre'].isin(empresas_sin_contactos)]
