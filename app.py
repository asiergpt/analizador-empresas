    ...
    with tab_profesional:
        ...
        if df_alumni.empty: st.warning("Datos no disponibles.")
            ...
            if col_nom:
                ...
                else: lista_prof = sorted(df_alumni[col_nom].head(50).unique().tolist())
                ...
                if lista_prof:
                    ...
                    if sel_prof and col_emp:
                        ...
                        if st.button("Ir a Empresa ->", key="go_prof", use_container_width=True):
                            ...
                            st.rerun()
                elif search_prof: st.warning("No encontrado.")
...
# Filtramos nombres inválidos como "nan" o vacíos para que no busquen basura
...
if selected_empresa and df_main is not None:
    ...
    if not row_data.empty:
        ...
        # 4. ALUMNI (Lógica existente)
        st.markdown('<div class="section-title">Networking (Alumni)</div>', unsafe_allow_html=True)
        # ... (Mantén tu código de alumni existente aquí si lo deseas, o usa el simplificado abajo)
        ...
        if col_matriz and not df_alumni.empty:
            ...
            if not match.empty:
                ...
                st.info(f"Se encontraron {num} contacto(s) en esta empresa.")
                # Aquí puedes volver a pegar tu lógica de tabs y tablas de alumni original
                render_table(match[['Nombre', 'Cargo', 'jerarquía']].head(10))
            else:
                st.info("No hay contactos alumni directos.")

    # --- CASO 2: EMPRESA NO ENCONTRADA (Lo que pediste) ---
    else:
        ...
        if not df_alumni.empty:
            ...
            # Usamos 'astype(str)' para evitar fallos si hay nans flotantes
    ...
    # --- PANTALLA DE BIENVENIDA (Sin selección) ---
    st.markdown("""<div style="text-align: center; margin-top: 50px;"><h1 style="color: #1F4E79; font-size: 3rem;">🔍 Analizador de Empresas</h1><p style="font-size: 1.2rem; color: #555;">Busca empresas y encuentra contactos clave.</p></div>""", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    c1.metric("🏢 Empresas", len(df_main) if df_main is not None else 0)
    c2.metric("🎓 Alumni", df_alumni['Nombre'].nunique() if not df_alumni.empty else 0)
