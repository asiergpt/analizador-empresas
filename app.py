...
st.markdown("""
    ...
    /* --- 1. KPIs SUPERIORES (Resumen Ejecutivo) --- */
    .kpi-card {
    ...
    .box-text b { color: #1F4E79; font-weight: 700; }
    ...
    /* --- 4. SECCIÓN TECH (EL RE-DISEÑO) --- */
    
    /* A) La Tabla Superior (CTO/Equipo) */
    .tech-hero-table {
        width: 100%;
    ...
    .tech-hero-table th:first-child, .tech-hero-table td:first-child {
    ...
    /* B) Las Tarjetas de Detalle (IA, Cloud, Perfil) */
    .tech-detail-card {
    ...
    /* Tablas Generales */
    .custom-table { width: 100%; border-collapse: collapse; margin-top: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
    .custom-table th { 
        background-color: #1F4E79; 
        color: white; 
        padding: 15px; 
    ...
    [data-testid="stSidebar"] label { font-size: 1.2rem !important; font-weight: 600; }
...
@st.cache_data
def load_data():
    # 1. Cargar Empresas (Este sigue siendo CSV normal porque son datos públicos)
    try:
    ...
    # 2. Cargar Alumni (DESCIFRADO AL VUELO) 🔐
    try:
        # A) Buscamos la llave en los secretos de la nube
        # NOTA: Si esto falla localmente es porque no tienes el archivo secrets.toml, 
        # pero funcionará en la nube cuando configuremos el paso 4.
        key = st.secrets["encryption_key"]
        cipher_suite = Fernet(key)
        
        # B) Leemos el archivo encriptado
        with open('alumni_seguro.enc', 'rb') as file:
            encrypted_data = file.read()
            
        # C) Desciframos los datos en la memoria RAM
        decrypted_data = cipher_suite.decrypt(encrypted_data)
        
        # D) Convertimos los bytes descifrados en un DataFrame de Pandas
        df_alumni = pd.read_csv(io.BytesIO(decrypted_data), sep=';', encoding='utf-8')
        
        # Limpieza normal de siempre
        ...
        # Si no hay llave o falla, creamos un dataframe vacío para que la app no se rompa
        # st.error(f"Error de seguridad: {e}") # Descomenta esto si quieres ver el error
...
if df_main is None:
    ...
    if selected_empresa:
        ...
        with col3:
            st.markdown(f'<div class="kpi-card"><div class="kpi-label">💰 Viabilidad Salarial >80k</div><div class="kpi-value {clase_viabilidad}">{veredicto_clean}</div></div>', unsafe_allow_html=True)
        ...
        # 4. MADUREZ TECNOLÓGICA (CORREGIDO)
        ...
        # Detalles en Tarjetas (SIN sangría para evitar errores de renderizado)
        html_tech_details = f"""
