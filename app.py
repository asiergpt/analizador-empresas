...
# --- INTERFAZ PRINCIPAL ---
if df_main is None:
    st.error("❌ No se encontró el archivo 'euskadi_navarra_dollar.csv'.")
else:
    # --- SIDEBAR ---
    ...
    if len(empresa_list) > 0:
        # Intenta encontrar ANGULAS AGUINAGA SAU en la lista
        default_index = 0
        if "ANGULAS AGUINAGA SAU" in empresa_list:
            default_index = empresa_list.index("ANGULAS AGUINAGA SAU")
        
        selected_empresa = st.sidebar.selectbox(
    ...
    if selected_empresa:
        ...
        clasificacion_raw = get_val(row, 'veredicto_final')
        clasificacion = "TOP" if "TIBURÓN" in clasificacion_raw.upper() else clasificacion_raw# Expectativa salarial
        ...
        with col3:
            st.markdown(f'<div class="kpi-card"><div class="kpi-label">💰 Viabilidad salarial</div><div class="kpi-value {clase_viabilidad}">{veredicto_clean}</div></div>', unsafe_allow_html=True)
