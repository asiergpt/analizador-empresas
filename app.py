        ...
        if not match.empty:
            ...
            with t1:
                if 'jerarquía' in match.columns:
                    ...
                    # Agrupar y contar
                    counts = match.groupby('jerarquía').size().reset_index(name='Total')
                    # Recalcular el orden sobre el resultado agrupado
                    counts['order'] = counts['jerarquía'].map(order_map).fillna(5)
