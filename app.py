...
# --- 9. PÁGINA: BUSCAR PROFESIONALES ---
    ...
    with st.sidebar:
        ...
        # Filtro: Nombre de Empresa (MEJORADO - Obtiene de ambas columnas)
        if not df_alumni.empty:
            # Obtiene empresas de AMBAS columnas (matriz_einforma Y nombre_dba)
            empresas_matriz = set(
                df_alumni['nombre_matriz_einforma'].fillna('').astype(str).str.strip().unique()
            )
            empresas_dba = set(
                df_alumni['nombre_dba'].fillna('').astype(str).str.strip().unique()
            )
            empresas_combinadas = sorted(
                (empresas_matriz | empresas_dba) - {'', 'nan', '-'}
            )
            
            empresa_seleccionada = st.selectbox(
                "Empresa", 
                ["Todas"] + empresas_combinadas,
                index=0
            )
        ...
        # Filtro: Función (AHORA ES MULTISELECT como Jerarquía)
        if not df_alumni.empty:
            funciones = sorted(
    ...
    if df_personas.empty:
        ...
        # Filtro por empresa (MEJORADO - Busca en ambas columnas)
        if st.session_state.f_personas_empresa:
            df_personas = df_personas[
                (df_personas['nombre_matriz_einforma'].astype(str).str.strip() == 
                 str(st.session_state.f_personas_empresa).strip()) |
                (df_personas['nombre_dba'].astype(str).str.strip() == 
                 str(st.session_state.f_personas_empresa).strip())
            ]
        ...
        # Filtro por función (AHORA MULTISELECT)
