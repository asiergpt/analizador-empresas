...
# --- 4. CARGA DE DATOS (VERSIÓN CORREGIDA) ---
@st.cache_data
def load_data():
    df_main = None
    ...
    if os.path.exists(file_empresas):
        try: df_main = pd.read_csv(file_empresas, sep=';', encoding='utf-8')
            ...
            try: df_main = pd.read_csv(file_empresas, sep=';', encoding='latin-1')
                ...
                try: df_main = pd.read_csv(file_empresas, sep=',', encoding='utf-8') # Intento con coma
                except: pass
    ...
    if os.path.exists(file_alumni) and "encryption_key" in st.secrets and Fernet:
        try:
            ...
            # st.sidebar.error(f"Error desencriptando: {e}") # Opcional: comentar para no ensuciar
            pass
    ...
    # --- LIMPIEZA CRÍTICA DE DATOS (ESTO SOLUCIONA TU ERROR) ---
    if not df_main.empty:
        # 1. Limpieza de Empleados: Forzar a número
        if 'numero_empleados' in df_main.columns:
            # Convierte todo lo que no sea número a NaN (Not a Number)
            df_main['numero_empleados'] = pd.to_numeric(df_main['numero_empleados'], errors='coerce')
            # Rellena los huecos con 0 y convierte a entero
            df_main['numero_empleados'] = df_main['numero_empleados'].fillna(0).astype(int)
        else:
            # Si la columna no existe por alguna razón, la creamos con ceros
            df_main['numero_empleados'] = 0

        # 2. Limpieza de IA (para evitar error en el selectbox)
