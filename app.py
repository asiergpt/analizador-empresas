...
# --- 9. PÁGINA: BUSCAR PERSONAS ---
    ...
    with st.sidebar:
        ...
        st.session_state.f_personas_nombre = st.text_input("Nombre Persona", 
                                                            value=st.session_state.f_personas_nombre)
        
        # Filtro: Nombre de Empresa
        if not df_alumni.empty:
            empresas_list = sorted(
                df_alumni['nombre_matriz_einforma'].fillna(df_alumni['nombre_dba']).dropna().unique().tolist()
            )
            empresa_default_idx = 0
            if st.session_state.f_personas_empresa in empresas_list:
                empresa_default_idx = empresas_list.index(st.session_state.f_personas_empresa) + 1
            
            empresa_seleccionada = st.selectbox("Empresa", 
                                                ["Todas"] + empresas_list,
                                                index=empresa_default_idx)
            st.session_state.f_personas_empresa = "" if empresa_seleccionada == "Todas" else empresa_seleccionada
        
        # Filtro: Provincia de Empresa
        provs_empresas = sorted(df_main['provincia'].dropna().unique().tolist())
        st.session_state.f_personas_provincia = st.multiselect("Provincia Empresa", 
                                                                provs_empresas, 
                                                                default=st.session_state.f_personas_provincia)
        
        # Filtro: Jerarquía
        if not df_alumni.empty:
            jerarquias = sorted(df_alumni['jerarquía'].dropna().unique().tolist())
            st.session_state.f_personas_jerarquia = st.multiselect("Nivel Jerárquico", 
                                                                    jerarquias, 
                                                                    default=st.session_state.f_personas_jerarquia)
        
        # Filtro: Función
        st.session_state.f_personas_funcion = st.text_input("Función", 
                                                             value=st.session_state.f_personas_funcion)
    ...
    if df_personas.empty:
        ...
        if st.session_state.f_personas_empresa:
            df_personas = df_personas[
                (df_personas['nombre_matriz_einforma'].astype(str).str.contains(
                    st.session_state.f_personas_empresa, case=False, na=False)) |
                (df_personas['nombre_dba'].astype(str).str.contains(
                    st.session_state.f_personas_empresa, case=False, na=False))
            ]
        ...
        if st.session_state.f_personas_provincia:
            ...
            df_personas = df_personas[
                df_personas['nombre_matriz_einforma'].isin(empresas_provincia) |
                df_personas['nombre_dba'].isin(empresas_provincia)
            ]
        
        # Filtro por jerarquía
        if st.session_state.f_personas_jerarquia:
            df_personas = df_personas[df_personas['jerarquía'].isin(st.session_state.f_personas_jerarquia)]
        
        # Filtro por función
        if st.session_state.f_personas_funcion:
            df_personas = df_personas[df_personas['función'].astype(str).str.contains(
                st.session_state.f_personas_funcion, case=False, na=False)]
        ...
        if not df_page.empty:
            for index, row in df_page.iterrows():
                ...
                # Badge de jerarquía
                if "top" in jerarquia.lower():
                    badge_html = '<span class="badge-top">Top Management</span>'
                elif "middle" in jerarquia.lower():
                    badge_html = '<span class="badge-mid">Middle Management</span>'
                else:
                    badge_html = '<span class="badge-entry">Entry Level</span>'
                
                # Expander con info de la persona
                with st.expander(f"👤 **{nombre}** - {funcion}"):
                    c1, c2, c3, c4 = st.columns(4)
                    c1.metric("Empresa", empresa[:25] + "..." if len(empresa) > 25 else empresa)
                    c2.metric("Provincia", provincia)
                    c3.metric("Función", funcion[:20] + "..." if len(funcion) > 20 else funcion)
                    c4.markdown(f'<div style="text-align: center; margin-top: 10px;">{badge_html}</div>', unsafe_allow_html=True)
                    
                    st.write("")
                    
                    # Tabla con detalles completos
                    df_detalle = pd.DataFrame({
                        "Campo": ["Nombre", "Empresa", "Provincia", "Función", "Jerarquía"],
                        "Valor": [nombre, empresa, provincia, funcion, jerarquia]
                    })
