...
def inject_css():
    ...
    .content-box { 
        ...
        min-height: 220px; /* Altura mínima forzada para alineación */
        display: flex;
...
def generate_mock_data():
    data_emp = {
        ...
        'SECTOR_NOMBRE': ['Tecnología', 'Industria', 'Consultoría', 'Transporte', 'Ingeniería', 'Automoción', 'Energía'],
        'financiacion_publica_detalle': ['Hazitek', 'No', 'CDTI', 'Europea', 'No', 'Gobierno Vasco', 'No'],
        'solvencia_txt': ['Alta', 'Media', 'Alta', 'Muy Alta', 'Alta', 'Muy Alta', 'Alta']
    }
...
# ----------------- DETALLE -----------------
    ...
    if not row_data.empty:
        ...
        # Layout equilibrado (50/50 o 30/70 visualmente similar al contenido)
        c_sec, c_act = st.columns([1, 1])
        with c_sec:
             st.markdown(f"""<div class="content-box"><span class="box-header">Sector</span>{get_val(r, 'SECTOR_NOMBRE')}</div>""", unsafe_allow_html=True)
        with c_act:
             st.markdown(f"""<div class="content-box"><span class="box-header">Actividad Principal</span>{get_val(r, 'actividad_resumen')}</div>""", unsafe_allow_html=True)
