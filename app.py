...
def navigate_to(page):
    ...
    if page == 'explorer':
        ...
        # Opcional: reiniciar página de tabla al volver
        # st.session_state.current_page = 0 

def select_company(empresa_nombre):
    st.session_state.selected_empresa = empresa_nombre
    st.session_state.page = 'detail'
    # Hack para scrollear arriba: al cambiar de estado 'page', Streamlit suele renderizar desde 0.
...
# ----------------- EXPLORADOR -----------------
    ...
    # Asegurar que la página actual es válida tras filtrar
    ...
    st.write(f"**Resultados:** {total_items} empresas encontradas. (Página {st.session_state.current_page + 1} de {max(1, total_pages)})")
    ...
    if not df_page.empty:
        ...
        for index, row in df_page.iterrows():
            ...
            # Cambiado nombre de botón
            if c[5].button("➕ Más detalles", key=f"btn_{index}"):
...
# ----------------- DETALLE -----------------
    ...
    # Al entrar aquí, estamos renderizando desde arriba, simulando scroll top.
    ...
    if not row_data.empty:
        ...
        if col_matriz and not df_alumni.empty:
            ...
            if not match.empty:
                ...
                with t1:
                    # Tabla resumen por categoría (jerarquía)
                    if 'jerarquía' in match.columns:
                        try:
                            summary = match['jerarquía'].value_counts().reset_index()
                            summary.columns = ['Categoría', 'Total']
                            render_table(summary)
                        except:
                            st.write("No se pudo generar el resumen.")
                    else:
                        st.info("Columna de jerarquía no disponible.")
