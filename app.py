...
# --- 7. PÁGINA: EXPLORADOR ---
    ...
    # Export CSV
    if total_items > 0:
        st.markdown('<div class="export-button-container"></div>', unsafe_allow_html=True)
        ...
        with col_exp1:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"bingo_empresas_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
...
# --- 8. PÁGINA: DETALLE ---
    ...
    if not row_data.empty:
        ...
        if not match.empty:
            ...
            # TAB 1: RESUMEN POR JERARQUÍA
            with t1:
                if 'jerarquía' in df_sorted.columns:
                    # Agrupar y contar preservando orden
                    hierarchy_order = {
                        'Top Management': 1,
                        'Middle Management': 2,
                        'Entry Level': 3,
                    ...
                    summary_data = []
                    for hierarchy in ['Top Management', 'Middle Management', 'Entry Level', 'Entry Level/Others', 'Otros']:
                        count = len(df_sorted[df_sorted['jerarquía'] == hierarchy])
                        if count > 0:
                            summary_data.append({
                                'Nivel': hierarchy,
                                'Total': count
                    ...
                    if summary_data:
                        ...
                        for _, row_c in df_summary.iterrows():
                            ...
                            if "Top" in str(cat):
                                badge = "badge-top"
                            elif "Middle" in str(cat):
                                badge = "badge-mid"
                            else:
                                badge = "badge-entry"
                            html_sum += f'<tr><td><span class="{badge}">{cat}</span></td><td style="text-align: center;"><b>{row_c["Total"]}</b></td></tr>'
                ...
                if 'jerarquía' in df_display.columns:
                    def get_badge(hierarchy):
                        if pd.isna(hierarchy):
                            return '-'
                        hierarchy = str(hierarchy).strip()
                        if "Top" in hierarchy:
                            return f'<span class="badge-top">{hierarchy}</span>'
                        elif "Middle" in hierarchy:
                            return f'<span class="badge-mid">{hierarchy}</span>'
                        else:
                            return f'<span class="badge-entry">{hierarchy}</span>'
